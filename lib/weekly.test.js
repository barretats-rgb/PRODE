...
const { weeklyTop } = require("./weekly.js");

test("weeklyTop: top 3 ordenado, con gap (puntos a la punta)", () => {
  const top = weeklyTop(playersW, "s1");
  assert.deepStrictEqual(top, [
    { id: "u2", name: "Sofi",   points: 12, exact: 3, gap: 0 },
    { id: "u1", name: "Joaco",  points: 12, exact: 2, gap: 0 },
    { id: "u3", name: "Pancho", points: 5,  exact: 1, gap: 7 },
  ]);
});

test("weeklyTop: respeta el limit y descarta a los que no sumaron", () => {
  assert.deepStrictEqual(weeklyTop(playersW, "s1", 2).map(p => p.id), ["u2", "u1"]);
  assert.deepStrictEqual(weeklyTop(playersW, "s2").map(p => p.id), ["u1"]); // sólo u1 sumó en s2
});

test("weeklyTop: nadie sumó / lista vacía o nula → []", () => {
  assert.deepStrictEqual(weeklyTop(playersW, "s5"), []);
  assert.deepStrictEqual(weeklyTop([], "s1"), []);
  assert.deepStrictEqual(weeklyTop(null, "s1"), []);
});
