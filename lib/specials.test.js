...
test("constantes: 6 especiales de a 5 puntos", () => {
  assert.deepStrictEqual(SPECIAL_KEYS, ["campeon","subcampeon","goleador","arquero","continente","fairplay"]);
  assert.strictEqual(SPECIAL_POINTS, 5);
});

test("specialsLocked: antes del 16 jun 18:00 CR → false, después → true", () => {
  assert.strictEqual(specialsLocked(Date.parse("2026-06-16T17:59:00-06:00")), false);
  assert.strictEqual(specialsLocked(Date.parse("2026-06-16T18:00:00-06:00")), true);
});
...
test("fanout: múltiples claves confirmadas suman", () => {
  const r = scoreSpecialsFanout({ campeon: "ARG", fairplay: "GER" }, [
    { playerId: "u1", campeon: "ARG", fairplay: "GER" },
  ]);
  assert.deepStrictEqual(r.perPlayer, { u1: { specialsPoints: 10 } });
  assert.deepStrictEqual(r.perPrediction[0].awarded, { campeon: 5, fairplay: 5 });
});
